...
class TradingModel:
    ...
    def getData(self):
        ...
        params = '?pair=' + ticker + '&interval=5'
        url = base + endpoint + params
        #download the data
        json_string = requests.get(url)
        a_json = json.loads(json_string.text)
        #creat pandas df
    ...
    def strategy(self):
        ...
        for i in range(1, len(df['close'])):
			if df['sma40'][i] > df['low'][i] and (df['sma40'][i] - df['low'][i]) > 0.03 * df['low'][i] and df['obv'][i] < 0:
				buy_signals.append([df['time'][i], df['low'][i]])
        #check for sell points
			if df['obv'][i] >= 100000 and df['rsi'][i] >= 65:
